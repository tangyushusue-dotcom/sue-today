  ...
  return (
    <PageShell title="欢迎来到 Sue 🌸">
      ...
      <Card className="mt-6 p-5">
        <h2 className="font-serif text-lg font-medium">🎯 你的运动目标？</h2>
          ...
          {GOALS.map((g) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm transition ${
                goal === g.key ? "bg-rose/30 font-medium text-ink" : "bg-blush/70 text-ink/80 hover:bg-blush"
              }`}
            >
              {g.emoji} {g.label}
            </button>
      ...
      <Card className="mt-4 p-5">
        <h2 className="font-serif text-lg font-medium">📅 每周运动几天？</h2>
        <div className="mt-3 flex flex-wrap gap-2">
          {DAYS.map((d) => (
            <button
              key={d}
              ...
              className={`grid h-10 w-10 place-items-center rounded-full text-sm transition ${
                days === d ? "bg-rose/30 font-medium text-ink" : "bg-blush/70 text-ink/80 hover:bg-blush"
              }`}
            >
              {d}
            </button>
      ...
      {/* 城市 */}
      <Card className="mt-4 p-5">
        <h2 className="font-serif text-lg font-medium">📍 你在哪个城市？</h2>
        <p className="mt-1 text-xs text-muted">用于自动获取天气，帮你选穿搭</p>
        <div className="mt-3 flex flex-wrap gap-2">
          {CITIES.map((c) => (
            <button
              key={c}
              ...
              className={`rounded-full px-3 py-1.5 text-sm transition ${
                actualCity === c ? "bg-rose/30 font-medium text-ink" : "bg-blush/70 text-ink/80 hover:bg-blush"
              }`}
            >
              {c}
            </button>
      ...
      <div className="mt-6 flex gap-3">
        <button onClick={save} disabled={saving} className="flex-1 rounded-2xl bg-rose/90 py-3 font-medium text-white shadow-card transition hover:bg-rose disabled:opacity-60">
          {saving ? "保存中…" : "保存并开始"}
        </button>
        <button onClick={() => router.push("/")} className="rounded-2xl bg-white/70 px-6 py-3 text-sm text-muted shadow-card transition hover:text-ink">
          先跳过
        </button>
      </div>
    </PageShell>
